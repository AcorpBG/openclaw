...
function resolveModelOverridePolicy(
  ...
  return {
    ...
    // New OpenAI transport/format controls stay opt-in to avoid surprise behavior changes.
    allowInstructions: allow(overrides?.allowInstructions, false),
    allowStream: allow(overrides?.allowStream, false),
    allowResponseFormat: allow(overrides?.allowResponseFormat, false),
    allowSpeed: allow(overrides?.allowSpeed),
    allowStreamFormat: allow(overrides?.allowStreamFormat, false),
  };
}
...
export async function textToSpeechStream(params: {
  ...
  try {
    ...
    return {
      ...
      provider: "openai",
      get outputFormat() {
        return streamResult.outputFormat;
      },
      get voiceCompatible() {
        return streamResult.outputFormat === "opus";
      },
    };
  } catch (err) {
    return {
