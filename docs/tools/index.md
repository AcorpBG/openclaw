...
- For ACP runtime, `model` and `thinking` overrides currently apply only to `agentId: "codex"` on the `acpx` backend. Supported `thinking` values are `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, plus `none`; `adaptive` is rejected.
